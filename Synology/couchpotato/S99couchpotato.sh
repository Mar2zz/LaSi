...
#
# Edited by Mar2zz
# Latest version: http://dl.dropbox.com/u/18712538/couchpotato/S99couchpotato.sh
# 
...
# startup args
DAEMON_OPTS=" $APP_PATH/CouchPotato.py -d --config=$CFG_PATH/config.ini --datadir=$CFG_PATH"
...
case "$1" in
    ...
    update)
        if ! run_update | grep 'Already up-to-date'; then
            /opt/etc/init.d/S99couchpotato.sh restart
        fi
        ;;
    status)
        if daemon_status; then
