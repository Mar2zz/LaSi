...
PORT=$(grep -m1 port $APP_PATH/config.ini | sed 's/http_port = //g');
USERNAME=$(grep -m1 username $APP_PATH/config.ini | sed 's/http_username = //g');
PASSWORD=$(grep -m1 password $APP_PATH/config.ini | sed 's/http_password = //g');
...
# Shutdown
echo "Stopping $DESC ..."
wget -q --spider $AUTH http://localhost:$PORT/shutdown/ > /dev/null
